...
export default function PageMemberCompany({
  ...
  const userStatus = user?.employerInfo?.userStatus === "ACTIVE"; //- active mới cho call api

  const { data: listMemberCompany, isLoading: isLoadingMemberCompany } =
    useGetMemberCompany(userStatus);
  ...
  const { mutateAsync: kickMemberMutation } = useKickMemberCompany();

  const handleDelete = async (userId: string) => {
    try {
      const res = await kickMemberMutation(userId);

      if (res?.isError) return;

      SoftSuccessSonner(res?.message);
    } catch (error) {
      console.log("error kick member: ", error);
    }
  };
