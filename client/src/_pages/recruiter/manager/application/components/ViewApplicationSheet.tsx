...
import { Mail, Briefcase, CalendarClock, FileText, ClipboardList } from "lucide-react";
import { format } from "date-fns";
import { vi } from "date-fns/locale";
import { generateStarRating, generateStatusOptions } from "../application-jobColumn";
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        {isLoading ? (
          ...
          <div className="flex-1 flex justify-center items-center">
            <span className="text-muted-foreground">Không tìm thấy đơn ứng tuyển</span>
          </div>
        ) : (
            ...
            <div className="w-full md:w-1/3 xl:w-[400px] border-b md:border-b-0 md:border-r bg-background overflow-y-auto outline-none p-5 flex flex-col gap-6">
              ...
              <div className="flex items-center gap-4">
                <Avatar className="w-16 h-16 border">
                  <AvatarImage src={typeof application.userId === 'object' ? application.userId.avatar || undefined : undefined} />
                  <AvatarFallback className="text-lg bg-primary text-primary-foreground">
                    {typeof application.userId === 'object' ? application.userId.name?.charAt(0) : "U"}
                  </AvatarFallback>
                </Avatar>
                <div className="min-w-0">
                  <h3 className="font-semibold text-lg truncate">
                    {typeof application.userId === 'object' ? application.userId.name : "Người dùng đã xóa"}
                  </h3>
              ...
              <div className="grid grid-cols-2 gap-4 bg-muted/30 p-4 rounded-xl border border-border/50">
                <div>
                  <p className="text-[11px] font-medium uppercase text-muted-foreground mb-1.5">Trạng thái</p>
                  <div>{generateStatusOptions(application.status)}</div>
                </div>
                <div>
                  <p className="text-[11px] font-medium uppercase text-muted-foreground mb-1.5">Độ tiềm năng</p>
                  <div>
                    {application.rating && application.rating > 0 
                      ? generateStarRating(application.rating) 
                      : <span className="text-sm font-medium text-muted-foreground">Chưa đánh giá</span>
                    }
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 p-4 rounded-xl border border-border/50 bg-card">
                <div>
                  ...
                  <p className="font-medium text-sm leading-snug">
                     {typeof application.jobId === 'object' ? (application.jobId.title?.vi || application.jobId.title?.en) : "Công việc đã xóa"}
                  </p>
                </div>
                ...
                {application.status === "INTERVIEW" && application.interviewTime && (
                  <div className="pt-3 border-t">
                    <p className="text-[11px] font-medium text-blue-600 dark:text-blue-400 uppercase mb-1.5 flex items-center gap-1.5">
                      <CalendarClock className="w-3.5 h-3.5" /> Thời gian phỏng vấn
                    </p>
                    <p className="text-sm font-medium text-blue-700 dark:text-blue-300">
                      {format(new Date(application.interviewTime), "dd/MM/yyyy • HH:mm", { locale: vi })}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2.5">
                ...
                <div className="text-sm text-foreground bg-muted/20 p-3.5 rounded-lg border border-border/50 whitespace-pre-wrap leading-relaxed min-h-[100px]">
                  {(typeof application.coverLetter === 'object' ? application.coverLetter.vi : application.coverLetter) || <span className="italic text-muted-foreground">Không đính kèm thư giới thiệu.</span>}
                </div>
              </div>
              ...
              {application.recruiterNote && (
                <div className="space-y-2.5">
                  ...
                  <div className="text-sm bg-yellow-50 dark:bg-yellow-900/10 text-yellow-800 dark:text-yellow-200/80 p-3.5 border border-yellow-200 dark:border-yellow-900/30 rounded-lg whitespace-pre-wrap">
                    {typeof application.recruiterNote === 'object' ? application.recruiterNote.vi : application.recruiterNote}
                  </div>
                </div>
              )}
            </div>
