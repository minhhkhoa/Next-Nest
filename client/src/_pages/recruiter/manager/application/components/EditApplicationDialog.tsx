  ...
  useEffect(() => {
    if (application && open) {
      form.reset({
        ...
        recruiterNote: typeof application.recruiterNote === 'object' ? application.recruiterNote?.vi : (application.recruiterNote || ""),
        interviewTime: application.interviewTime
          ? new Date(application.interviewTime).toISOString().slice(0, 16)
          : "",
        rejectionReason: typeof application.rejectionReason === 'object' ? application.rejectionReason?.vi : (application.rejectionReason || ""),
      });
    }
                ...
                {application && (
                  <div className="bg-muted/30 p-4 rounded-lg border border-border/50 text-sm space-y-3">
                    <div className="grid grid-cols-[100px_1fr] gap-2">
                      <span className="font-semibold text-foreground">Ứng viên:</span>
                      <span className="text-muted-foreground break-words">
                        {typeof application.userId === 'object' ? application.userId.name : "Người dùng ẩn"}
                      </span>
                    </div>
                    <div className="grid grid-cols-[100px_1fr] gap-2">
                      <span className="font-semibold text-foreground">Email:</span>
                      <span className="text-muted-foreground break-words">{application.email}</span>
                    </div>
                    <div className="grid grid-cols-[100px_1fr] gap-2">
                      <span className="font-semibold text-foreground">Công việc ứng tuyển:</span>
                    ...
                    {application.coverLetter && (
                      <div className="pt-2">
                        <span className="font-semibold text-foreground block mb-2">Thư giới thiệu:</span>
                        <div className="text-muted-foreground p-3 bg-background rounded-md border min-h-[60px] whitespace-pre-wrap leading-relaxed">
                          {typeof application.coverLetter === 'object' ? application.coverLetter.vi : application.coverLetter}
                        </div>
                      </div>
                    )}
                  </div>
