            ...
            >
              <div className="relative overflow-hidden bg-slate-200 h-64 md:h-72 lg:h-80">
                <Image
                  ...
                  alt={getLang(firstNews.title) || t("FeaturedNews")}
                  fill
              ...
              <div className="p-5 md:p-6 flex flex-col flex-1">
                <p className="text-xs md:text-sm font-semibold text-indigo-400 uppercase tracking-wide mb-2">
                  {getLang(firstNews.cateNewsID[0]?.name) || "News"}
                </p>
                ...
                <div className="flex-1 flex flex-col min-w-0 justify-between">
                  <div>
                    <p className="text-xs md:text-sm font-semibold text-indigo-400 uppercase tracking-wide mb-1">
                      {getLang(newsItem.cateNewsID[0]?.name) || "News"}
                    </p>
...
function BlockStyle1({ data }: { data: ResultListNewsType }) {
  ...
  const formatDateInputLocal = (dateStr: string) => {
      ...
      <div className="flex justify-between items-center">
        <div className="mb-6 md:mb-8">
          <span className="text-xl md:text-3xl lg:text-4xl font-bold text-primary text-balance">
            {getLang(data.nameCate.title)}
          </span>
          <div className="h-1 w-[165px] md:w-[248px] bg-primary rounded-full mt-2"></div>
        </div>
        <Link
          href={`/cate-news/${generateSlugUrl({
            name: getLang(data.nameCate),
        ...
        {data.listNews.slice(0, 4).map((news) => (
          ...
          >
            ...
            <p className="text-xs md:text-sm text-gray-500 my-1 flex items-center gap-1">
              ...
              {news.createdBy.name} •{" "}
              {formatDateInputLocal(news.createdAt.toString())}
            </p>

            <p className="line-clamp-3 text-shadow-md">
              {getLang(news.summary)}
            </p>
